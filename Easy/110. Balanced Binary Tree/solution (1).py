...
class Solution:
    ...
    def isBalanced(self, root: TreeNode) -> bool:
        if not root:
            ...
            deep_differ = abs(Solution._getDeep(root.left) - Solution._getDeep(root.right)) <= 1
            balance_child = self.isBalanced(root.left) and self.isBalanced(root.right)
            return deep_differ and balance_child

    @staticmethod
    def _getDeep(node: TreeNode) -> int:
        if not node:
            return 0
        else:
            return max(Solution._getDeep(node.left), Solution._getDeep(node.right)) + 1