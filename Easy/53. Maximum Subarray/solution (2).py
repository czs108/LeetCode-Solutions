...
class Solution:
    ...
    def maxSubArray(self, nums: List[int]) -> int:
        return Solution._maxSubArray(nums, 0, len(nums) - 1)

    @staticmethod
    def _maxSubArray(nums: List[int], left: int, right: int) -> int:
        if left >= right:
            return nums[left]
        else:
            mid = left + (right - left) // 2
            return max(Solution._maxSubArray(nums, left, mid),
                Solution._maxSubArray(nums, mid + 1, right),
                Solution._maxCrossingSubArray(nums, left, mid, right))

    @staticmethod
    def _maxCrossingSubArray(nums: List[int], left: int, mid: int, right: int) -> int:
